/**
 * CheckerMakananOrders.tsx - Checker's Kitchen/Makanan Order View
 * 
 * View-only interface for Checker to monitor kitchen orders.
 * Similar to KitchenOrders but without cook selection functionality.
...
function FoodIcon({ onClick }: { onClick: () => void }) {
  return (
    <div className="size-[80px] cursor-pointer hover:opacity-80 transition-opacity" data-name="food" onClick={onClick}>
      <CheckerEyeIcon />
    </div>
  );
}

export default function CheckerMakananOrders() {
  const { getOrders, finishItem, completeOrder, assignWaiter, assignWaiterToItem, markItemDelivered, markDelivered } = useOrders();
  const orders = getOrders('kitchen');
  ...
  const handleItemInteraction = (item: MenuItem) => {
    ...
    setWaiterPanel({
      isOpen: true,
      orderId: orders.find(o => o.items.some(i => i.id === item.id))?.id || null,
      orderName: orders.find(o => o.items.some(i => i.id === item.id))?.name || '',
      itemId: item.id,
      itemName: item.name
    });
  };
  ...
  return (
    <div className="min-h-screen bg-[#4D236E]">
      ...
      <div className="bg-[#4d236e] relative h-[100px] px-[44px] py-[10px]">
        ...
        <p className="absolute font-['Poppins:Regular',sans-serif] leading-[normal] left-[133px] not-italic text-[28px] text-white top-[17px]">Section Makanan (Checker)</p>
                    ...
                    >
                      ...
                      {item.waiter && !item.itemDelivered && (
                        <>
                          ...
                          <button
                            onClick={() => {
                              markItemDelivered('kitchen', order.id, item.id);
                            }}
                    ...
                    <button
                      onClick={() => {
                        if (allItemsHaveWaiters) {
                          markDelivered('kitchen', order.id);
                        }
                      }}
        ...
        onSelectWaiter={(waiterName) => {
          if (waiterPanel.orderId) {
            ...
            if (waiterPanel.itemId) {
              assignWaiterToItem('kitchen', waiterPanel.orderId, waiterPanel.itemId, waiterName);
            } else {
              assignWaiter('kitchen', waiterPanel.orderId, waiterName);
            }
          }
