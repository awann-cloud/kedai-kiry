...
export default function PinEntryChecker() {
  const navigate = useNavigate();
  const [pin, setPin] = useState<string[]>(['', '', '', '']);
  const [error, setError] = useState(false);
  const correctPin = getPinForDepartment('checker');
    ...
    if (firstEmpty !== -1) {
      ...
      if (firstEmpty === 3) {
        ...
        if (enteredPin === correctPin) {
          // Correct PIN - navigate to checker home
          setTimeout(() => navigate('/checkerhome'), 300);
        } else {
