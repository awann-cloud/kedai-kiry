/**
 * SearchReceiptSidebarChecker.tsx - Checker Receipt Search Sidebar
 * 
 * A sliding sidebar for Checker pages with quick navigation between:
 * - All Departments
 * - Makanan (Kitchen)
 * - Bar
 * - Snack
...
import CheckerDepartmentIcon from "../imports/CheckerDepartmentIcon";
...
import HomeBarIcon from '../imports/HomeBarIcon';
import HomeSnackIcon from '../imports/HomeSnackIcon';
import HomeKitchenIcon from '../imports/HomeKitchenIcon';

interface SearchReceiptSidebarCheckerProps {
  isOpen: boolean;
  orders: Order[];
  onClose: () => void;
  currentPage: 'all' | 'makanan' | 'bar' | 'snack';
}

export default function SearchReceiptSidebarChecker({
  isOpen,
  orders,
  onClose,
  currentPage
}: SearchReceiptSidebarCheckerProps) {
  ...
  const getDepartmentInfo = (department: string) => {
    switch (department) {
      case 'kitchen':
        return { label: 'MAKANAN', color: '#61428C', bgColor: '#8b6dac' };
      case 'bar':
        return { label: 'BAR', color: '#2196F3', bgColor: '#64B5F6' };
      case 'snack':
        return { label: 'SNACK', color: '#FF9800', bgColor: '#FFB74D' };
      default:
        return { label: 'UNKNOWN', color: '#757575', bgColor: '#BDBDBD' };
    }
  };
  ...
  // Handle search - expand matching orders
  const handleSearch = (query: string) => {
    ...
    if (matchingOrderIds.length > 0) {
      ...
      // Scroll to the first expanded order
      setTimeout(() => {
  ...
  const handleNavigation = (page: 'all' | 'makanan' | 'bar' | 'snack') => {
    const routes = {
      all: '/checkerorders',
      makanan: '/checkermakananorders',
      bar: '/checkerbarorders',
      snack: '/checkersnackorders'
    };
    // Navigate with sidebar=open query parameter to keep sidebar open
    navigate(`${routes[page]}?sidebar=open`);
  };
    ...
    <>
      ...
      >
        ...
        <div className="bg-[#4d226c] h-[100px] shadow-[0px_10px_10px_0px_rgba(0,0,0,0.25)] relative">
          {/* Checker Eye Icon - Click to navigate to CheckerHome */}
          <div 
            className="absolute left-[33px] size-[70px] top-[15px] cursor-pointer hover:opacity-80 transition-opacity" 
            onClick={handleHomeClick}
          >
            <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 80 80">
              <circle cx="40" cy="40" fill="#61428C" r="40" />
              <g transform="translate(18, 18) scale(0.55)">
                <path d="M73 40.84C66.2 28.89 54.39 21 40 21s-26.2 7.89-33 19.84c6.8 11.95 18.61 19.84 33 19.84s26.2-7.89 33-19.84zM40 54c-7.73 0-14-6.27-14-14s6.27-14 14-14 14 6.27 14 14-6.27 14-14 14zm0-22c-4.42 0-8 3.58-8 8s3.58 8 8 8 8-3.58 8-8-3.58-8-8-8z" fill="white" />
              </g>
            </svg>
          </div>
        ...
        {/* Search Bar */}
        <div className="px-[41px] pt-[24px] pb-[16px]">
          <input
            type="text"
            value={searchQuery}
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button
              ...
              className={`flex-1 h-[38px] rounded-lg flex items-center justify-center transition-all ${
                currentPage === 'all'
                  ? 'bg-white text-[#4d226c] shadow-md'
                  : 'bg-[#61428C] text-white hover:bg-[#7a5aa5]'
              }`}
            >
              <span className="text-[11px] font-['Poppins:Bold',sans-serif]">ALL</span>
            </button>

            {/* MAKANAN Button */}
            <button
              onClick={() => handleNavigation('makanan')}
              className={`flex-1 h-[38px] rounded-lg flex items-center justify-center transition-all ${
                currentPage === 'makanan'
                  ? 'bg-white text-[#4d226c] shadow-md'
                  : 'bg-[#61428C] text-white hover:bg-[#7a5aa5]'
            ...
            >
              <span className="text-[11px] font-['Poppins:Bold',sans-serif]">MAKANAN</span>
            </button>
            ...
            >
              <span className="text-[11px] font-['Poppins:Bold',sans-serif]">BAR</span>
            </button>

            {/* SNACK Button */}
            <button
              onClick={() => handleNavigation('snack')}
              className={`flex-1 h-[38px] rounded-lg flex items-center justify-center transition-all ${
                currentPage === 'snack'
                  ? 'bg-white text-[#4d226c] shadow-md'
                  : 'bg-[#61428C] text-white hover:bg-[#7a5aa5]'
            ...
            >
              <span className="text-[11px] font-['Poppins:Bold',sans-serif]">SNACK</span>
            </button>
          </div>
        </div>

        {/* Scrollable Content */}
        <div className="h-[calc(100vh-258px)] overflow-y-auto px-[26px] space-y-6 pb-8">
              ...
              >
                ...
                <div className="relative shadow-[0px_8px_12px_0px_rgba(0,0,0,0.4)]">
                  ...
                  >
                    ...
                    {/* Department Icon */}
                    <div className="absolute left-[197px] top-[22px] w-[40px] h-[35px] bg-[rgba(0,0,0,0)]">
