...
export default function PinEntryMakanan() {
  const navigate = useNavigate();
  const [pin, setPin] = useState<string[]>(['', '', '', '']);
  const [error, setError] = useState(false);
  const correctPin = getPinForDepartment('kitchen');
    ...
    if (firstEmpty !== -1) {
      ...
      if (firstEmpty === 3) {
        ...
        if (enteredPin === correctPin) {
          // Correct PIN - navigate to kitchen orders
          setTimeout(() => navigate('/makanan'), 300);
        } else {
  ...
  return (
    <div className="bg-[#4a1e69] relative w-full h-screen flex">
      ...
      <div className="w-[46%] relative flex flex-col items-center justify-center">
        {/* Sparkle */}
        <div className="absolute top-[5.2%] left-1/2 -translate-x-1/2 w-[clamp(80px,11.72vw,120px)] h-[clamp(80px,15.63vh,120px)] mask-alpha mask-intersect mask-no-clip mask-no-repeat" style={{ maskImage: `url('${imgGroup}')` }}>
            ...
            <g id="food">
              ...
              <g id="Food Iocn">
                <mask height="50" id="mask0_9_95" maskUnits="userSpaceOnUse" style={{ maskType: "luminance" }} width="58" x="11" y="17">
                  <g id="303cbfc917">
                    <path d={CookChefIconPaths.pb41c280} fill="white" id="Vector" />
                  </g>
                </mask>
                <g mask="url(#mask0_9_95)">
                  <g id="Group">
                    <path d={CookChefIconPaths.pdb93600} fill="white" id="Vector_2" />
                  </g>
                  <path d={CookChefIconPaths.p242b800} fill="white" id="Vector_3" />
                </g>
              </g>
            </g>
          </svg>
