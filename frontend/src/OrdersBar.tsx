...
import IconsProfileBar from './imports/IconsProfileBar'; // Bar icon for profile/header
import { useOrders } from './contexts/OrderContext';
import IconsReceiptBar from './imports/IconsReceiptBar'; // Bar icon for receipts
import IconReceiptBarNew from './imports/IconReceiptBarNew'; // New Figma-imported bar receipt icon
import SearchReceiptSidebarBar from './components/SearchReceiptSidebarBar';

// Bar icon component for header
function BarIcon({ onClick }: { onClick: () => void }) {
  return (
    <div className="w-[80px] h-[80px] flex-shrink-0 cursor-pointer" onClick={onClick}>
      <IconsProfileBar />
    </div>
  );
}
...
export default function BarOrders() {
  ...
  const orders = getOrders('bar'); // Get bar orders from shared state
  ...
  const handleStartItem = (orderId: string, itemId: string, cookName: string) => {
    startItem('bar', orderId, itemId, cookName);
  };

  const handleDoneItem = (orderId: string, itemId: string) => {
    finishItem('bar', orderId, itemId);
  };

  const handleFinishOrder = (orderId: string) => {
    completeOrder('bar', orderId);
  };
    ...
    <div className="min-h-screen bg-[#4D236E]">
      ...
      <div className="bg-[#4d236e] relative h-[100px] px-[44px] py-[10px]">
        <div className="absolute left-[44px] top-[10px]">
          <BarIcon onClick={() => setIsSidebarOpen(true)} />
        </div>
        <p className="absolute font-['Poppins:Regular',sans-serif] leading-[normal] left-[133px] not-italic text-[28px] text-white top-[17px]">Section Bar</p>
            ...
            <div key={order.id} id={`order-${order.id}`} className="flex-shrink-0 w-[391px]">
              <div className="bg-[#8b6dac] rounded-[19px] overflow-hidden shadow-lg h-[633px] relative">
                ...
                <div className="absolute top-[58px] left-0 right-0 bg-[#f7f7f9] rounded-[19px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] h-[89px] z-10">
                  <div className="absolute right-[18px] top-[40px] w-[22px] h-[35px] flex-shrink-0">
                    <IconReceiptBarNew />
                  </div>
