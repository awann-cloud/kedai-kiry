...
export default function PinEntrySnack() {
  const navigate = useNavigate();
  const [pin, setPin] = useState<string[]>(['', '', '', '']);
  const [error, setError] = useState(false);
  const correctPin = getPinForDepartment('snack');
    ...
    if (firstEmpty !== -1) {
      ...
      if (firstEmpty === 3) {
        ...
        if (enteredPin === correctPin) {
          // Correct PIN - navigate to snack orders
          setTimeout(() => navigate('/snackorders'), 300);
        } else {
