/**
 * CheckerBarOrders.tsx - Checker's Bar Order View
 * 
 * View-only interface for Checker to monitor bar orders.
 * Similar to BarOrders but without bartender selection functionality.
...
import IconReceiptBarNew from './imports/IconReceiptBarNew'; // New working Figma-imported bar receipt icon
  ...
  return (
    <div className="min-h-screen bg-[#4D236E]">
      ...
      <div className="bg-[#4d236e] relative h-[100px] px-[44px] py-[10px]">
        <BarIcon onClick={() => setIsSidebarOpen(!isSidebarOpen)} />
        <p className="absolute font-['Poppins:Regular',sans-serif] leading-[normal] left-[133px] not-italic text-[28px] text-white top-[17px]">Section Bar (Checker)</p>
                    ...
                    >
                      ...
                      {item.waiter && !item.itemDelivered && (
                        <>
                          ...
                          <button
                            onClick={() => {
                              markItemDelivered('bar', order.id, item.id);
                            }}
        ...
        onSelectWaiter={(waiterName) => {
          if (waiterPanel.orderId) {
            ...
            if (waiterPanel.itemId) {
              assignWaiterToItem('bar', waiterPanel.orderId, waiterPanel.itemId, waiterName);
            } else {
              assignWaiter('bar', waiterPanel.orderId, waiterName);
            }
          }
